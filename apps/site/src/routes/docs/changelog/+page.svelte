...
sudo systemctl restart glassmkr-crucible</code></pre>
    ...
    <section class="release" id="2026-05-13">
      ...
      <p>The programmatic API surface (account keys) is now uniformly gated by Pro plan. Free customers retain full web-dashboard access and full <em>read</em> API access for their own data; programmatic <em>writes</em> (channel CRUD, alert acknowledge / resolve, server CRUD, mutes, restore endpoints, key management) and Pro features (AI analysis, trend warnings) return <code>402 pro_required</code> when called via an API key on a Free plan. Web-dashboard sessions are unaffected at any tier. Full breakdown at <a href="/docs/api/tier-gating">/docs/api/tier-gating</a>.</p>
...
<style>
  ...
  pre { background: var(--surface); border: 1px solid var(--surface-border); border-left: 3px solid rgba(245, 166, 35, 0.35); border-radius: var(--radius-md); padding: 12px 14px; overflow-x: auto; margin: 0.5rem 0 0.75rem; }
